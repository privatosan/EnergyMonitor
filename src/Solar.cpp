...
static const std::chrono::minutes UPDATE_PERIOD(5);
static const uint32_t ADDRESSES = 3;
...
enum class Code
{
    ...
    CODE_PAC,   // AC power [mW]
    CODE_PIN,   // Power installed [mW
    CODE_PRL,   // AC power [%]
    CODE_CAC,   // Start ups
    CODE_FRD,   // ???
    CODE_SCD,   // ???
...
Solar::Solar()
    : m_stop(false)
{
    for (uint32_t index = 0; index < ADDRESSES; ++index)
    {
        std::ostringstream name;
        name << "solar_kwh" << index;
        m_channels.push_back(Channel(name.str(), 0, index, 0.0f, 1.0f));
    }
}
...
static std::vector<uint32_t> parseReply(uint32_t src, const std::string &reply, const std::vector<Code> codes)
{
    ...
    size_t index = 0;
    if (!reply.compare(index, expected.str().length(), expected.str()))
    ...
    for (auto&& code: codes)
    {
        ...
        if (!reply.compare(index, codeStr.length(), codeStr))
            throw std::runtime_error("Unexpected reply");
        ...
        uint32_t value = 0;
        std::istringstream(reply.substr(index)) >> value;
...
void Solar::threadFunction()
{
    ...
    while (!done)
    {
        for (uint32_t address = 0; address < ADDRESSES; ++address)
        {
            ...
            try
            {
                ...
                request.setOpt(new curlpp::options::Url(IP_ADDR));
                request.setOpt(new curlpp::options::Port(PORT));
                request.setOpt(new curlpp::options::ReadStream(&msg));
                request.setOpt(new curlpp::options::InfileSize(msg.str().length()));
                ...
                m_channels[address].set(result[0]);
            }
